...
subtitle: "A complete tutorial from theory to working code"
author: "Hujie Wang"
date: "2026-01-20"
categories: [diffusion, flow-matching, generative-models, tutorial, cifar-10]
substack_url: "https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-917"
---
...
- **[Part 2](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-8ed)**: Vector fields and flows
- **[Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507)**: The marginalization trick and CFM loss
- **[Part 7](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-148)**: Training algorithms
- **[Part 9](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-956)**: U-Net architecture
...
- **Time conditioning**: Sinusoidal embeddings tell the network "where we are" in the flow (*t* ∈ [0,1])
...
Recall from [Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507) that flow matching learns a vector field to transport noise to data. Here's the intuition:
...
We use the **Gaussian CondOT path** — the same formulation from [Part 7](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-148):

$$x_t = \alpha_t \cdot z + \beta_t \cdot \epsilon$$

where:

- *z* ~ *p*_data is a real image
- ε ~ 𝒩(0, *I*) is noise
- αₜ = *t* and βₜ = 1 − *t* (linear schedule)

This gives us the **linear interpolation**:

$$x_t = t \cdot z + (1-t) \cdot \epsilon$$

![The linear path interpolates from noise to data. At t=0, we have pure noise ε; at t=1, the clean image *z* emerges.](linear-path-diagram.png)

::: {.callout-note appearance="simple"}
## Boundary Conditions

- At *t* = 0: αₜ = 0, βₜ = 1, so *x*ₜ = ε (pure noise)
- At *t* = 1: αₜ = 1, βₜ = 0, so *x*ₜ = *z* (clean data)
...
This is the **optimal transport** path — the straight line from noise to data. Compared to Gaussian variance-preserving paths used in DDPM ([Part 5](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-f2f)):

1. **Constant vector field**: *u*ₜ = *z* − ε doesn't change with time
2. **Straighter trajectories**: Fewer ODE steps needed (10-50 vs 1000)
3. **Same math**: The marginalization trick still applies
:::

**The conditional vector field** is remarkably simple. Taking the derivative of *x*ₜ = *t* · *z* + (1−*t*) · ε:

$$u_t(x_t | z) = \frac{d x_t}{dt} = z - \epsilon$$

This is **constant in time**! The direction from noise to image is simply their difference.

**The CFM loss** (from [Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507)) trains the network to predict this vector field:

$$\mathcal{L}_{\text{CFM}} = \mathbb{E}_{t, z, \epsilon} \left[ \| u_t^\theta(x_t) - (z - \epsilon) \|^2 \right]$$

The implementation is straightforward — sample *t*, compute *x*ₜ, and minimize MSE between predicted and target vector field:
...
Recall from [Part 8](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-6a4) that for Gaussian paths, there's a beautiful connection to Bayes' rule. The **conditional score** decomposes as:
...
During training, we **randomly drop class labels** with probability η (typically 10%). When dropped, we replace with a "null" class token. This teaches one model to do two things:

- Real label *y*: Model learns conditional *u*ₜ^θ(*x*ₜ|*y*)
- Null label ∅: Model learns unconditional *u*ₜ^θ(*x*ₜ|∅)

### Sampling: Guided Velocity

At inference, we **blend** both predictions:

$$\tilde{u}_t = u_t^\theta(x_t | \emptyset) + w \cdot \bigl(u_t^\theta(x_t | y) - u_t^\theta(x_t | \emptyset)\bigr)$$

Rearranging: *ũ*ₜ = (1−*w*) · *u*ₜ^θ(*x*ₜ|∅) + *w* · *u*ₜ^θ(*x*ₜ|*y*)

The **guidance scale** *w* controls conditioning strength:

- *w* = 0: Pure unconditional (ignores class)
- *w* = 1: Standard conditional
- *w* = 2-4: Typical range — sharper, more on-topic
- *w* > 8: Oversaturated, artifacts
...
2. Compute the flow matching loss
...
To generate images, we solve the ODE (recall [Part 2](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-8ed) — flows are generated by integrating vector fields):

$$\frac{dx_t}{dt} = u_t^\theta(x_t | y)$$

Starting from noise ε at *t*=0 and integrating to *t*=1:

1. Sample initial noise ε ~ 𝒩(0, *I*)
...
2. **Gaussian CondOT path**: *x*ₜ = *t* · *z* + (1−*t*) · ε with constant vector field *u*ₜ = *z* − ε
3. **CFM loss**: Train to predict the vector field
4. **Classifier-free guidance**: Blend conditional and unconditional predictions
5. **ODE sampling**: Integrate from noise (*t*=0) to image (*t*=1)
...
- **Diffusion Transformers (DiT)**: Replace U-Net with a transformer ([Part 10](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-686))

...
$$\textcolor{blue}{\mathcal{L}_{FM}(\theta)} = \mathbb{E}_{t, z \sim p_{data}, x_t \sim p_t(\cdot|z)} \|\textcolor{purple}{u_\theta(x_t, t)} - \textcolor{green}{u_t(x_t|z)}\|^2$$ {#eq-flow-matching-loss}

where $\textcolor{blue}{\text{loss}}$ measures error between $\textcolor{purple}{\text{predicted velocity}}$ and $\textcolor{green}{\text{target velocity}}$.

**Score matching** (Denoising Score Matching loss):
$$\textcolor{blue}{\mathcal{L}_{DSM}(\theta)} = \mathbb{E}_{t, z \sim p_{data}, x_t \sim p_t(\cdot|z)} \|\textcolor{purple}{s_\theta(x_t, t)} - \textcolor{green}{s_t(x_t|z)}\|^2$$ {#eq-score-matching-loss}

where $\textcolor{blue}{\text{loss}}$ measures error between $\textcolor{purple}{\text{predicted score}}$ and $\textcolor{green}{\text{target score}}$.

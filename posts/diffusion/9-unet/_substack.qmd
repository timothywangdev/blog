...
subtitle: "How skip connections and time embeddings enable image generation"
author: "Hujie Wang"
date: "2026-01-15"
categories: [diffusion, flow-matching, generative-models, u-net, architecture]
substack_url: "https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-956"
---
...
1. Handle the *same noisy image* differently depending on time *t* (aggressive denoising at *t* ≈ 0, gentle refinement at *t* ≈ 1)
...
In this post, we'll cover **U-Net** — the dominant architecture for diffusion models. For transformer-based approaches (DiT), see [Part 10: Diffusion Transformers](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-686).
...
Recall from [Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507) and [Part 7](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-148) that our goal is to learn a neural network that can predict either:

1. **Vector field**: *u*_θ(*x*ₜ, *t*, *y*) ≈ *u*ₜ(*x*ₜ) for flow matching — points in the direction of clean data

2. **Score function**: *s*_θ(*x*ₜ, *t*, *y*) ≈ ∇_{*x*ₜ} log *p*ₜ(*x*ₜ) for score-based diffusion — gradient of the log-probability, also points toward high-density regions

Here *y* is an optional conditioning input (class label, text prompt, or other control signal).

Both are fundamentally the same task: given noisy image *x*ₜ, timestep *t*, and condition *y*, output a same-sized tensor that tells us "how to update each pixel."
...
**Mathematical reason**: A vector field *u*ₜ : ℝᵈ → ℝᵈ maps each point in space to a **direction in the same space**. Think of a weather map showing wind: at each (*x*, *y*) location, there's a wind velocity vector pointing in some direction. The velocity lives in the same 2D space as the location.

**Physical meaning**: The vector field tells us "how to update *x*ₜ" when we solve the ODE d*x*ₜ/d*t* = *u*ₜ(*x*ₜ). To add this update to *x*ₜ (which is *H* × *W* × *C*), the update *u*ₜ(*x*ₜ) must also be *H* × *W* × *C* — we need one "velocity" value per pixel per channel.

**Score function case**: Similarly, *s*ₜ(*x*ₜ) = ∇_{*x*ₜ} log *p*ₜ(*x*ₜ) is a gradient with respect to *x*ₜ, which by definition has the same shape as *x*ₜ.
...
1. **Encoder**: Progressively downsample the input (512×512 → 256×256 → 128×128 → ... → 8×8) — reduces spatial resolution while increasing channels, extracts increasingly abstract features

2. **Bottleneck**: Process at the lowest resolution (e.g., 8×8×512) — rich semantic representation, but tiny spatial size

3. **Decoder**: Progressively upsample back to original size (8×8 → ... → 512×512) — reconstructs the output from the compressed representation
...
4. **Skip connections**: The gray arrows — copy features across the U
...
**Preventing "forgetting"**: Deep networks tend to lose low-level features as information passes through many layers. Skip connections reintroduce these features, ensuring the decoder doesn't have to "remember" what the encoder saw — it can access it directly.
:::

![Semantic vs spatial information: decoder features know "what" (semantic understanding, but blurry), encoder features know "where" (spatial precision, but abstract). Skip connections combine both for precise reconstruction.](semantic-spatial-fusion.png)

![Skip connection mechanism: encoder feature map (128 channels of high-resolution spatial details) is concatenated with decoder feature map (256 channels of upsampled semantic features) along the channel dimension, resulting in 384 total channels. Spatial dimensions stay the same.](skip-connection-concat.png)
...
![Comparison: Without skip connections (left), the bottleneck destroys spatial details leading to blurry reconstruction. With skip connections (right), high-resolution features bypass the bottleneck, enabling sharp detail reconstruction.](skip-connection-comparison.png)

### Adaptations for Diffusion

The original U-Net was designed for segmentation (image → mask). For diffusion, we need (image, time, condition) → velocity field. Here's how it's adapted:
...
**What is an embedding?** A mapping from a simple input (like a number *t* or a word) to a high-dimensional vector that captures its "meaning." Here: scalar *t* ∈ [0,1] → vector *e*ₜ ∈ ℝ⁵¹² (or whatever dimension matches the network's channels).

![Time embedding transforms a single scalar timestep value into a rich high-dimensional vector that encodes "what this timestep means" for the network.](time-embedding.png)

**How it's done** — Sinusoidal position encoding:

$$
t \to [\sin(2^0 \pi t), \cos(2^0 \pi t), \sin(2^1 \pi t), \cos(2^1 \pi t), \sin(2^2 \pi t), \cos(2^2 \pi t), \ldots] \to \text{MLP} \to e_t
$$

Think of it like encoding the number using multiple "frequencies" — low frequencies (2⁰ = 1) capture coarse time differences, high frequencies (2¹⁰ = 1024) capture fine distinctions.
...
3. **Generalizable**: Unlike learned embeddings, sinusoidal encodings work for *any* timestep value — a model trained with 50 steps still works with 1000 steps at inference

**How it's used**: The time embedding *e*ₜ is added to the feature maps at every resolution level of the U-Net, allowing each layer to "know" what time it is and adjust its processing accordingly.
...
- Time: *t* → sinusoidal + MLP → *e*ₜ ∈ ℝ⁵¹²
- Condition (if any): *y* → embedding layer → *e*_y ∈ ℝ⁵¹²
- Combine: *e* = *e*ₜ + *e*_y (element-wise addition)
...
**Step 2**: Inject at every residual block

![Time and condition injection in a U-Net residual block: embedding *e* is linearly projected and added to the feature map *h* between convolutions, allowing the conditioning signal to modulate the block's processing.](time-injection-resblock.png)
...
U-Net is trained with standard diffusion objectives (see [Part 7](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-148)):

**Flow matching** (Conditional Flow Matching loss):
$$\mathcal{L}_{FM}(\theta) = \mathbb{E}_{t, z \sim p_{data}, x_t \sim p_t(\cdot|z)} \|u_\theta(x_t, t) - u_t(x_t|z)\|^2$$

**Score matching** (Denoising Score Matching loss):
$$\mathcal{L}_{DSM}(\theta) = \mathbb{E}_{t, z \sim p_{data}, x_t \sim p_t(\cdot|z)} \|s_\theta(x_t, t) - s_t(x_t|z)\|^2$$

::: {.callout-note appearance="simple"}
**Note**: We train by regressing against **conditional** targets (*u*ₜ(*x*ₜ|*z*) conditioned on data point *z*), but the network learns to approximate the **marginal** vector field *u*ₜ(*x*ₜ) needed for generation. This is the marginalization trick from [Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507) — training on conditional targets is equivalent to learning the marginal.
:::

The architecture doesn't change the learning objective — only how we parameterize the function *u*_θ(*x*ₜ, *t*, *y*) or *s*_θ(*x*ₜ, *t*, *y*).
...
You might think: "predict the velocity field *u*ₜ." But there are equivalent formulations that work better in practice:

- **Noise prediction**: Predict the noise ε that was added
- **Data prediction**: Predict the clean image *x*₀
...
1. **Noise prediction** (ε-parameterization): Network outputs ε_θ(*x*ₜ, *t*) ≈ ε where *x*ₜ = αₜ*x*₀ + σₜε — most common for DDPM-style training, works well across all timesteps

2. **Velocity prediction** (*v*-parameterization): Network directly predicts *v*_θ(*x*ₜ, *t*) ≈ *u*ₜ(*x*ₜ) — natural for flow matching, better for consistency models

3. **Data prediction** (*x*₀-parameterization): Network predicts *x̂*₀ = *f*_θ(*x*ₜ, *t*) — sometimes easier to interpret (direct image prediction), can be unstable at high noise levels

**Which to use?** ε-prediction is the safe default. Some recent work uses *v*-prediction for improved stability.
...
1. **Latent Diffusion Models (LDM)** — **Most important innovation**

   ![Latent Diffusion Model architecture from Rombach et al. (2022). Left: images are encoded to latent space *z* via encoder ℰ. Middle: diffusion happens in latent space, with conditioning (text, semantic maps, etc.) injected via cross-attention. Right: decoder 𝒟 reconstructs the image from denoised latents.](latent-diffusion-original.png)

   ::: {.callout-tip appearance="simple"}
   ## Why Latent Space?

   Running diffusion directly on 512×512 images is prohibitively expensive. The key insight: **perceptual compression** via a pretrained VAE removes imperceptible high-frequency details, letting the diffusion model focus on semantic content.
   :::
...
::: {.callout-note appearance="simple"}
**Looking for Diffusion Transformers?** See [Part 10: Diffusion Transformers](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-686) for how transformers (DiT) are adapted for diffusion models.
:::
...
We've seen how the mathematical theory of diffusion models (learning *u*ₜ(*x*ₜ, *t*) or *s*ₜ(*x*ₜ, *t*)) translates into practical neural network architectures.
...
- **Condition injection**: Time and class embeddings are combined (*e* = *e*ₜ + *e*_y) and injected into every residual block
...
- Formula: ε-prediction + sinusoidal time embedding + cross-attention for text → state-of-the-art text-to-image
- Latent diffusion (VAE + U-Net) makes high-resolution generation practical
:::

**What's next?** In [Part 10: Diffusion Transformers](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-686), we'll see how transformers (DiT) offer an alternative architecture with better scaling properties. Then we'll explore **sampling algorithms** — numerical methods for solving the learned ODE/SDE to transform noise into images.

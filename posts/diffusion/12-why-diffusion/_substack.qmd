...
substack_url: "https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-9d9"
---
...
- **Why it fails**: MSE-optimal prediction is mathematically guaranteed to be the mean 𝔼[*y*|*x*], which falls *between* modes and is often invalid
...
**Image generation**: Input "a cat sitting" → Infinitely many cats (different breeds, poses, lighting, backgrounds...)

**Robot control**: Input current state → Multiple valid actions (go left OR right around obstacle)

**Video prediction**: Input current frame → Many possible futures (object moves in any direction)
...
$$
\begin{aligned}
\mathbb{E}[(Y - a)^2] &= \mathbb{E}[(Y - \mathbb{E}[Y] + \mathbb{E}[Y] - a)^2] \\
&= \text{Var}(Y) + 0 + (\mathbb{E}[Y] - a)^2
\end{aligned}
$$

**Why does the cross-term vanish?** The middle term contains 𝔼[*Y* − 𝔼[*Y*]], which is the expected deviation from the mean — always zero by definition.
...
Since variance is fixed (the data is what it is), minimizing MSE is equivalent to minimizing (𝔼[*Y*] − *a*)² — a parabola with minimum at *a** = 𝔼[*Y*].
:::
...
![Mode averaging: the mean of two valid modes is often invalid](mode-averaging.png)

Here's what this looks like when we actually train models on a 2D bimodal distribution:

![Direct regression with MSE collapses to the mean (center), while flow matching successfully captures both modes.](direct-vs-flow-comparison.png)

### Real-World Consequences

**Image generation**: Mode averaging produces blurry images (average of all possible images)

**Video prediction**: Mode averaging produces ghostly, transparent objects (superposition of futures)

**Robot control**: Mode averaging produces invalid actions (e.g., go THROUGH obstacle instead of around)

**VAE reconstructions**: Mode averaging produces loss of fine details, smoothed textures
...
- Image classification: Each image has ONE true class
- Depth estimation: Each scene has ONE true depth
- Pose estimation: Person has ONE true pose
- Speech recognition: Each utterance has ONE transcription
- Object detection: Objects have ONE true location

In these tasks, 𝔼[*y*|*x*] = *y*_true because there's only one correct answer. The mean IS the answer.

**MSE fails when targets are multimodal (many valid answers):**

- Image generation: Input "a cat" → Infinitely many valid cats
- Super-resolution: Many valid high-freq details
- Future prediction: Many possible futures
- Imitation learning: Multiple valid strategies
- Inpainting: Many valid completions
...
**GANs**: Discriminator penalizes blurry outputs; generator must commit to ONE mode. Trade-off: Mode collapse, training instability.

**Autoregressive**: Generate one token at a time; each step is unimodal given context. Trade-off: Slow sequential generation.

**VAE + adversarial loss**: Add discriminator to VAE to enforce sharpness. Trade-off: More complex training.

**Mixture Density Networks**: Explicitly predict mixture of Gaussians. Trade-off: Must pre-specify number of modes.

**Normalizing Flows**: Learn invertible transform; exact likelihood. Trade-off: Architecture constraints.

**Discretization (VQ-VAE)**: Convert to tokens, use classification per token. Trade-off: Quantization artifacts.

**Diffusion / Flow Matching**: Iterative refinement; each step is simple. Trade-off: Slow sampling (many steps).
...
- Parti (2022): 20B params, encoder-decoder Transformer, FID 3.22
- LlamaGen (2024): Vanilla LLaMA architecture, 3.1B params, FID 2.18
- **VAR** (2024): NeurIPS 2024 Best Paper, 20x faster, **FID 1.73**
- DiT (diffusion, 2023): FID 2.27 for comparison
...
**2025 update**: Hybrid approaches like **HART** (MIT/NVIDIA) combine AR + diffusion, achieving **9x speedup** over pure diffusion with comparable quality.
:::

::: {.callout-note appearance="simple"}
## Why Diffusion Won (For Now)

**The turning point**: In 2021, Dhariwal & Nichol published "Diffusion Models Beat GANs on Image Synthesis", achieving FID 2.97 on ImageNet 128×128 (vs BigGAN's 6.02). By 2022, Stable Diffusion, DALL-E 2, and Imagen cemented diffusion's dominance.

**Why diffusion beat GANs:**

- **Training**: Diffusion is stable — single denoising objective. GANs are unstable — adversarial min-max game.
- **Diversity**: Diffusion has high recall, covers full distribution. GANs suffer mode collapse under truncation.
- **Scaling**: Diffusion scales smoothly to larger models. GANs require careful tuning at scale.
...
The main diffusion downside — slow sampling — is being addressed by consistency models (1-4 step generation) and distillation.
...
1. Sample initial noise *x*₀ ~ 𝒩(0, *I*)
2. Follow the learned vector field from *t*=0 to *t*=1
3. Arrive at a generated sample *x*₁

### Why This Works: The Three Key Insights

#### Insight 1: The Velocity Prediction Has a Unique Answer

Even when the data distribution is multimodal, the **conditional expected velocity** is well-defined and unique. For the Gaussian CondOT path ([Part 11](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-917)) where *x*ₜ = (1−*t*)*x*₀ + *t* · *x*₁:

$$
u_t^\theta(x_t) \approx \mathbb{E}[x_1 - x_0 \mid x_t]
$$

::: {.callout-note appearance="simple"}
This formula is for the **linear (OT) path**. For general Gaussian probability paths ([Part 3](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-bed)) with *x*ₜ = αₜ*z* + βₜε (where *z* is data and ε is noise), the target velocity is *u*ₜ(*x*ₜ|*z*) = α̇ₜ*z* + β̇ₜε. The key insight — that the velocity is well-posed — holds for **all** path choices.
:::

Why is the velocity well-posed? Because the spatial position of *x*ₜ **already encodes** information about which mode it's heading toward:

- Points near the LEFT mode → velocity points LEFT
- Points near the RIGHT mode → velocity points RIGHT

The network learns to **separate modes spatially**. At any given (*x*ₜ, *t*), there's one optimal direction to move.
...
![The learned vector field naturally separates modes: points on the left get pushed left, points on the right get pushed right. The spatial position encodes which mode to target.](velocity-field.png)

The vector field also evolves over time. Early on (*t*≈0), the field points outward from the noise region toward both modes. As *t* increases, points have committed to their trajectories and the field refines toward final positions.

![Vector field at t=0.0, 0.3, 0.6, 0.9. Early on (t≈0), the field points outward from the noise region toward both modes. As t increases, points have committed to their trajectories and the field refines toward final positions.](velocity-field-evolution.png)
...
- Different *x*₀ ~ 𝒩(0, *I*) → different trajectories → different final samples
...
- **Early** (*t* ≈ 0), high noise: Broad direction toward data manifold
- **Middle**: Progressive mode selection based on trajectory
- **Late** (*t* ≈ 1), low noise: Fine details within chosen mode
...
1. **Sample noise** *x*₀ ~ 𝒩(0, *I*) — this random sample determines WHICH cat we'll generate
...
**VAE**: Mode-covering strategy (cover ALL modes) → Diverse but blurry (averaging)

**GAN**: Mode-seeking strategy (focus on FEW modes) → Sharp but mode collapse

**Diffusion**: Mode-covering + iterative refinement → Sharp AND diverse

![VAE, GAN, and Diffusion represent different trade-offs: VAE covers all modes but is blurry, GAN is sharp but suffers mode collapse, Diffusion achieves both sharpness and diversity through iterative refinement.](vae-gan-diffusion-comparison.png)
...
![The Push-T task illustrates mode averaging failure: direct regression averages left and right approaches, causing the robot to crash through the obstacle. Diffusion Policy commits to one mode and succeeds.](push-t-comparison.png)

**Observed failure modes from the paper:**
...
**Diffusion Policy** represents the policy as a conditional denoising process:
...
- Block Pushing (push 2 blocks): 94% (+32% over BET at 71%)
- Franka Kitchen (4+ objects): 96% (+213% over BET at 44%)
...
- **ACT** (2023, 80M params): CVAE + Transformer, faster inference
- **Octo** (2024, 93M params): Open-source generalist, 800k trajectories
- **RDT-1B** (2024, 1.2B params): Largest diffusion foundation model, 46 datasets
- **π₀** (2024, 3.3B params): VLM + flow matching, 50Hz dexterous control
...
The **Conditional Flow Matching** loss is (from [Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507)):

$$
\mathcal{L}_{\text{CFM}} = \mathbb{E}_{t, z, x}\left[\|u_t^\theta(x) - u_t^{\text{target}}(x|z)\|^2\right]
$$

where *z* ~ *p*_data is a data sample and *x* is drawn from the conditional path *p*ₜ(*x*|*z*).

For the **Gaussian CondOT path** we use throughout this series ([Part 7](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-148)):

$$
x_t = t \cdot z + (1-t) \cdot \epsilon, \quad \epsilon \sim \mathcal{N}(0, I)
$$

the target velocity simplifies to *u*ₜ^target(*x*|*z*) = *z* − ε.

This looks like MSE regression! Why doesn't it suffer from mode averaging?

**Because the target (*z* − ε) is conditioned on a specific (*z*, ε) pair.**

During training:

- We sample a specific noise ε and specific data point *z*
- The target velocity *z* − ε is **unique** for this pair
...
Training with the simple conditional loss has the same gradients as training with the intractable marginal loss. See [Part 4](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-507) for the full derivation.
:::

...
subtitle: "From Vision Transformers to DiT — how transformers adapt for image generation"
author: "Hujie Wang"
date: "2026-01-17"
categories: [diffusion, flow-matching, generative-models, transformer, vit, dit, architecture]
substack_url: "https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-686"
---
...
Before understanding DiT, we need to understand **Vision Transformer (ViT)** — the architecture it's built on.
...
**The breakthrough**: Treat image patches as "visual words." A 224×224 image split into 16×16 patches becomes a sequence of (224/16)² = 196 tokens. Now standard transformer machinery applies!
:::
...
1. **Patchify**: Split image into non-overlapping *p* × *p* patches
   - Input: *H* × *W* × *C* image
   - Output: *N* = (*H*/*p*) × (*W*/*p*) patches, each of size *p*² · *C*
...
Just like words "fox" and "jumps" share semantic relationships, visual patches relate to one another:
...
**The math**: A 224×224 image = 50,176 pixels. Self-attention is O(*n*²), so attention would require 50,176² ≈ 2.5 billion operations per layer!

**Patches reduce this**: With 16×16 patches, we get only 196 tokens. Attention cost: 196² = 38,416 operations — a **65,000× reduction**.
...
- **Local bias**: Built-in (kernels are small)
- **Translation equivariance**: Built-in (same kernel everywhere)
- **Data efficiency**: Good (bias helps with limited data)
- **Scalability**: Limited (hard to scale past ~1B params)

**ViT has**:

- **Local bias**: None (attention is global)
- **Translation equivariance**: Must learn from data
- **Data efficiency**: Poor (needs lots of data)
- **Scalability**: Excellent (scales predictably)
...
**2. Operate in latent space** (see Latent Diffusion in [Part 9](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-956))
...
Recent work shows transformer architectures can exceed U-Net performance:

**U-Net**:

- **Scaling**: Diminishing returns past ~1B params
- **Design**: Complex (skip connections, multi-resolution)
- **Spatial bias**: Strong (convolutions)
- **Best for**: Limited compute, few data

**DiT**:

- **Scaling**: Predictable: 2× compute → better FID
- **Design**: Simple (stack identical blocks)
- **Spatial bias**: Weak (must learn from data)
- **Best for**: Massive compute, lots of data
...
**Example**: A 256×256 image with 16×16 patches becomes (256/16)² = 256 tokens. Now the transformer can process it just like a sentence of 256 words!

**The catch**: Attention is O(*n*²) in sequence length. A 512×512 image with 16×16 patches = 1024 tokens = 1 million attention computations per layer. This is why DiT operates in **latent space** (more on this below).
:::

![DiT pipeline: VAE latent (64×64×4) is split into patches, linearly embedded into tokens with positional encoding, processed through transformer blocks with AdaLN conditioning, then decoded back to latent space.](dit-patchify.png)

DiT adapts Vision Transformer (ViT) for diffusion:

1. **Patchify**: Split latent into *p* × *p* non-overlapping patches
...
**Pixel space**: 256×256×3 image with patch size 2 → (256/2)² = 16,384 tokens
**Attention cost**: 16,384² ≈ 270 million operations per layer!

**Latent space**: VAE compresses 256×256×3 → 32×32×4, then patch size 2 → (32/2)² = 256 tokens
**Attention cost**: 256² = 65,536 operations per layer

That's a **4000× reduction** in attention cost. Latent diffusion isn't optional for DiT — it's essential.
:::

![Latent Diffusion Model architecture from Rombach et al. (2022). Images are encoded to latent space *z* via encoder ℰ, diffusion operates in this compressed space with optional conditioning via cross-attention, then decoder 𝒟 reconstructs the output.](latent-diffusion-original.png)

**Patch size trade-off**:

- Patch Size 2: 1024 tokens from 64×64 latent → Best quality (FID 2.27), Highest compute
- Patch Size 4: 256 tokens → Good quality, Medium compute
- Patch Size 8: 64 tokens → Lower quality, Fastest
...
![AdaLN mechanism: timestep *t* and condition *y* are embedded and processed by an MLP to produce 6 parameters (γ, β for scale/shift, α for gating) that modulate how each transformer block processes features.](adaln-mechanism.png)

**How AdaLN works** — For each transformer block:

1. **Embed conditioning**: *c* = MLP(*e*ₜ + *e*_y) where *e*ₜ, *e*_y are time/class embeddings
2. **Predict 6 parameters**: (γ₁, β₁, α₁, γ₂, β₂, α₂) = Linear(*c*)
3. **Modulate attention branch**:
   - Scale and shift after LayerNorm: *x*' = γ₁ · LN(*x*) + β₁
   - Gate the residual: *x* = *x* + α₁ · Attention(*x*')
4. **Modulate MLP branch**:
   - Scale and shift: *x*' = γ₂ · LN(*x*) + β₂
   - Gate the residual: *x* = *x* + α₂ · MLP(*x*')

::: {.callout-tip appearance="simple"}
## Intuition: Why "Zero" in AdaLN-Zero?

The α (gating) parameters are initialized to **zero**. This means at initialization, each DiT block computes:
...
**Why this works so well**: Research shows that zero-initialized weights gradually converge to a Gaussian-like distribution during training. This smooth convergence is more stable than random initialization — the network learns to "turn on" conditioning gradually rather than starting with noisy modulation.
:::

**Why AdaLN-Zero wins** (ImageNet FID scores):

- **In-context** (append *t*, *y* as extra tokens): Cost O(*d*), FID 5.67
- **Cross-attention** (attend over condition embeddings): Cost O(*d*²), FID 3.75
- **AdaLN** (modulate LayerNorm scale/shift): Cost O(*d*), FID 2.98
- **AdaLN-Zero** (AdaLN + zero-init gating): Cost O(*d*), **FID 2.27**

**Note on text conditioning**: For free-form text prompts (not class labels), cross-attention works better than AdaLN. This is what Stable Diffusion uses — the text encoder outputs a sequence of embeddings, and each DiT block cross-attends to them. See [Part 9](https://aheadofrobotics.substack.com/p/diffusion-and-flow-matching-part-956) for more on how conditioning flows through the network.

## Comparison: U-Net vs DiT

![Architectural comparison: U-Net uses hierarchical multi-scale processing with skip connections (top) vs. DiT's flat architecture with global attention across all patches (bottom).](unet-vs-dit-comparison.png)
...
**Inductive bias**: U-Net has strong bias (convolution = local, translation-invariant); DiT has weak bias (attention = global, permutation-invariant)

**Scalability**: U-Net has diminishing returns past ~1B parameters; DiT has predictable scaling (2× compute → better FID)

**Training cost**: U-Net is lower (100s of GPU-days for Stable Diffusion); DiT is higher (1000s of GPU-days for DiT-XL/2)

**Sample quality at equivalent cost**: U-Net is better on small/mid budgets; DiT is better at massive scale

**Architecture complexity**: U-Net is high (skip connections, resolution-dependent attn); DiT is low (repeat identical blocks)

**Memory efficiency**: U-Net is better (hierarchical = smaller feature maps); DiT is worse (all patches processed at full resolution)

::: {.callout-warning appearance="simple"}
## Common Misconception

**"Transformers are always better than CNNs"** — Not true for diffusion! U-Nets remain state-of-the-art for many applications because they encode strong spatial priors. DiT only wins when you have enough data and compute to overcome its lack of inductive bias.
:::

**Empirical result**: DiT-XL/2 achieves FID 2.27 on ImageNet 256×256, beating the previous best (LDM) of 3.60. But this required training on 256 TPU-v3 cores — far more than U-Net baselines.

## DiT Model Configurations

DiT comes in four configurations borrowed from ViT:

- **DiT-S**: Hidden 384, Depth 12, Heads 6, 33M params, 0.4 Gflops
- **DiT-B**: Hidden 768, Depth 12, Heads 12, 130M params, 5.6 Gflops
- **DiT-L**: Hidden 1024, Depth 24, Heads 16, 458M params, 35.6 Gflops
- **DiT-XL**: Hidden 1152, Depth 28, Heads 16, 675M params, 119 Gflops
...
**Fréchet Inception Distance (FID)** does exactly this:
...
3. **Compute distance**: Calculate the Fréchet distance between the two Gaussians:
   ...
   where (μᵣ, Σᵣ) and (μ_g, Σ_g) are the mean/covariance of real and generated features
...
DiT has become the backbone for several state-of-the-art models. Recent research has also improved the core conditioning mechanism — **adaLN-Gaussian** initializes modulation weights from Gaussian distributions (instead of zero), yielding a 2.16% FID improvement over AdaLN-Zero.
...
**The bigger picture**: U-Net and DiT are different ways to parameterize the same math (*u*_θ(*x*ₜ, *t*)). The architecture choice is about trade-offs: inductive bias vs scalability, data efficiency vs ultimate performance.
:::

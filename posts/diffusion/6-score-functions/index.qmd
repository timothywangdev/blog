...
1. **Chain rule**: $\nabla \log f = \frac{\nabla f}{f}$
2. **[Marginalization](../3-probability-paths/)**: $p_t(x) = \int p_t(x|z) p_{data}(z) dz$ (law of total probability)
3. **Leibniz rule**$^*$: swap $\nabla_x$ and $\int_z$
...
Now that we understand conditional scores, let's prove why the SDE Extension Trick works. The key tool is the **Fokker-Planck equation**, which extends the [continuity equation (from Part 4)](../4-flow-matching-loss/#continuity-equation) to handle SDEs.
...
Therefore, if we set
$$
\textcolor{blue}{u_t^{sde}} = \textcolor{green}{u_t^{target}} + \textcolor{red}{\frac{\sigma_t^2}{2} \nabla \log p_t}
$$

where $\textcolor{blue}{\text{SDE drift}}$ = $\textcolor{green}{\text{ODE drift}}$ + $\textcolor{red}{\text{score correction}}$, then it satisfies the Fokker-Planck equation.
...
$$
\begin{aligned}
dX_t = [\textcolor{green}{u_t^{target}(X_t)} + \textcolor{red}{\frac{\sigma_t^2}{2} \nabla \log p_t(X_t)}] \, dt + \textcolor{orange}{\sigma_t dW_t}
\end{aligned}
$$

The $\textcolor{green}{\text{ODE drift}}$ provides direction, the $\textcolor{red}{\text{score correction}}$ counteracts diffusion to maintain the probability path, and $\textcolor{orange}{\text{noise}}$ adds stochasticity.
...
We proved the **SDE extension trick**: given an ODE with drift $u_t^{target}$, we can add arbitrary noise $\sigma_t dW_t$ while preserving the same probability path by correcting the drift:
$$
...
- **[Part 7](../7-training/)**: The complete training algorithm — how to implement score matching in practice
- **[Part 8](../8-guidance/)**: Sampling and inference — how to generate new samples from the trained model

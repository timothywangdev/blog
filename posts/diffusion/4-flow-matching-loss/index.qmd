...
We proved the **marginalization trick**: training on conditional targets $u_t^{target}(x|z)$ is mathematically equivalent to training on the intractable marginal $u_t^{target}(x)$. The key step was showing that the cross-term $\mathbb{E}[u_t^\theta \cdot u_t^{target}]$ simplifies when we expand the marginal definition — the intractable $p_t(x)$ cancels.

This gives us the **Conditional Flow Matching (CFM) loss**:
$$
\textcolor{blue}{\mathcal{L}_{CFM}(\theta)} = \mathbb{E}_{t,z,x}[\|\textcolor{purple}{u_t^\theta(x)} - \textcolor{green}{u_t^{target}(x|z)}\|^2]
$$

where $\textcolor{blue}{\text{loss}}$ measures the error between our $\textcolor{purple}{\text{learned velocity}}$ and the $\textcolor{green}{\text{target velocity}}$ (which points toward data point $z$).
...
We've now covered the complete flow matching framework — from vector fields to probability paths to the training loss. But remember from [Part 1](../1-intro/) that diffusion models take a different approach: they add **stochasticity** to the process.

In the [next post](../5-diffusion-sde/), we'll see how **diffusion models** generalize flow matching by adding a noise term to the ODE, turning it into an SDE. We'll discover:

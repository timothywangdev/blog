...
But how do we sample $x \sim p_t(x)$? Recall from [Part 3](../3-probability-paths/) that the marginal is defined as:
...
$$
\textcolor{blue}{\mathcal{L}_{\text{DDPM}}(\theta)} = \mathbb{E}_{t,z,\epsilon} \left[ \left\| \textcolor{purple}{\epsilon_t^\theta(\alpha_t z + \beta_t \epsilon, t)} - \textcolor{green}{\epsilon} \right\|^2 \right]
$$

where $\textcolor{blue}{\text{loss}}$ measures the error between $\textcolor{purple}{\text{predicted noise}}$ and $\textcolor{green}{\text{actual noise}}$ added during training.
...
This holds as long as $\dot{\alpha}_t \beta_t^2 - \dot{\beta}_t \beta_t \alpha_t \neq 0$. Factoring: $\beta_t(\dot{\alpha}_t \beta_t - \dot{\beta}_t \alpha_t)$. Recall from [Part 3](../3-probability-paths/) that $\alpha_t$ increases from 0 to 1 ($\dot{\alpha}_t > 0$) and $\beta_t$ decreases from 1 to 0 ($\dot{\beta}_t < 0$). Therefore $\dot{\alpha}_t \beta_t > 0$ and $-\dot{\beta}_t \alpha_t > 0$, so the second factor is strictly positive for $t \in (0, 1)$.
...
$$
\textcolor{blue}{u_t^\theta(x)} = \textcolor{orange}{\frac{\dot{\alpha}_t}{\alpha_t}} x + \textcolor{purple}{\left(\frac{\dot{\alpha}_t \beta_t^2}{\alpha_t} - \dot{\beta}_t \beta_t\right)} \textcolor{green}{s_t^\theta(x)}
$$

where $\textcolor{blue}{\text{velocity}}$ = $\textcolor{orange}{\text{scaling}}$ · position + $\textcolor{purple}{\text{coefficient}}$ · $\textcolor{green}{\text{score}}$.

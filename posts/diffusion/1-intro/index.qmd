...
Imagine dropping a leaf into a river. The current ([vector field](../2-flows/)) tells the leaf which way to move at each moment. If we design the currents correctly, leaves dropped anywhere in the "noise region" will flow toward the "data region."
...
Despite their different presentations, diffusion and flow matching are deeply related. Every diffusion model has an equivalent "probability flow" formulation that looks like flow matching. The [score function](../6-score-functions/) (gradient of log probability) in diffusion corresponds to the velocity field in flow matching.

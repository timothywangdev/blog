...
## Why Action Chunking Matters {#sec-why}

Imagine teaching a robot to pick up a mug. In traditional behavioral cloning (recall from [Part 1](../1-background/#sec-imitation)), the robot predicts one tiny action at a time — 50 micro-adjustments per second. Each prediction is an opportunity for error. Over a 10-second task, that's **500 chances to drift off course**. Errors compound exponentially, and the robot quickly enters states it never saw during training.
...
## Intuition: Chunking in Human Cognition
...
## The Problem with Single-Step Prediction {#sec-problem}

### Compounding Errors in Behavioral Cloning {#sec-compounding}
...
::: {.callout-tip appearance="simple"}
## Intuition: The Steering Wheel Analogy
...
### The Non-Markovian Problem {#sec-non-markovian}
...
## Action Chunking: The Core Idea {#sec-action-chunking}
...
$$
\begin{aligned}
\text{Standard BC:} \quad & \pi(a_t | o_t) && \text{(predict single action)} \\
\text{Action Chunking:} \quad & \pi(a_{t:t+k} | o_t) && \text{(predict k-step sequence)}
\end{aligned}
$$ {#eq-chunking-objective}
...
![Horizon reduction through action chunking: A 500-step task becomes only 5 decision points when predicting chunks of k=100 actions, dramatically reducing compounding errors.](horizon-reduction.png){#fig-horizon-reduction}

### Receding Horizon Control {#sec-receding-horizon}
...
::: {.callout-tip appearance="simple"}
## Intuition: Driving Down a Highway
...
## ACT Architecture {#sec-architecture}

Action Chunking with Transformers (ACT) was introduced in the 2023 paper ["Learning Fine-Grained Bimanual Manipulation with Low-Cost Hardware"](https://arxiv.org/abs/2304.13705) by Tony Z. Zhao, Vikash Kumar, Sergey Levine, and Chelsea Finn.$^{[1]}$ It combines action chunking with a Conditional Variational Autoencoder (CVAE) and Transformer architecture adapted from DETR (DEtection TRansformer).$^{[5]}$
...
### The Role of CVAE {#sec-cvae}

Why use a Variational Autoencoder instead of simple supervised learning?

#### The Multimodality Problem

Recall from [Part 1](../1-background/#sec-multimodality) that human demonstrations exhibit natural variability:
...
::: {.callout-tip appearance="simple"}
## Intuition: The Style Variable

Think of the latent variable $z$ as capturing *how* you perform a task, separate from *what* task you're doing. When placing a mug, $z$ might encode:
- Approach from left vs. right
- Fast, confident motion vs. slow, careful placement
- High arc trajectory vs. low, direct path
...
$$z \sim q_\phi(z | o_t, a_{t:t+k}) = \mathcal{N}(\mu_\phi(o_t, a_{t:t+k}), \sigma_\phi^2(o_t, a_{t:t+k}))$$ {#eq-cvae-encoder}

To enable backpropagation through this stochastic sampling, we use the **reparameterization trick**:

$$z = \mu + \sigma \cdot \epsilon, \quad \epsilon \sim \mathcal{N}(0, 1)$$ {#eq-reparameterization}
...
$$a_{t:t+k} \sim p_\theta(a_{t:t+k} | o_t, z)$$ {#eq-cvae-decoder}

The training loss has two terms:

$$\mathcal{L} = \mathcal{L}_{\text{reconst}} + \beta \cdot \mathcal{L}_{\text{KL}}$$ {#eq-cvae-loss}

where:
- **Reconstruction loss** ($L_1$ norm): $\mathcal{L}_{\text{reconst}} = \|a_{t:t+k} - \hat{a}_{t:t+k}\|_1$ ensures accurate action prediction
- **KL divergence**: $\mathcal{L}_{\text{KL}} = D_{KL}(q_\phi(z|o_t, a_{t:t+k}) \| \mathcal{N}(0, I))$ regularizes the latent distribution toward a standard Gaussian prior

**Inference:**

At deployment, the encoder is **not used** (we don't have ground-truth actions). Instead:

1. Sample $z \sim \mathcal{N}(0, 1)$ from the prior (or simply set $z=0$)
2. Pass $z$ and current observation through the decoder
3. Get the k-step action sequence

::: {.callout-warning}
## Common Misconception: "CVAE Generates Different Behaviors at Test Time"
...
**Encoder:**
- 4 layers, 8 attention heads, 512 hidden dimensions
- Input: concatenated vision features (1200×512 from 4 cameras), joint positions (512), and $z$ (512)
- Output: fused multi-modal representation (1202×512)

**Decoder:**
- 7 layers, 8 attention heads, 512 hidden dimensions
- Uses fixed sinusoidal positional embeddings as queries
- Cross-attends to encoder output
- Output: k×14 dimensional tensor (k timesteps × 14 joint positions for bimanual robot)

**Total parameters**: ~80 million (lightweight compared to VLA models with billions)

### Temporal Ensembling {#sec-temporal-ensembling}
...
![Temporal ensembling: At each timestep, multiple overlapping predictions exist for the same future action. These are combined using exponential weights that favor recent predictions.](temporal-ensembling.png){#fig-temporal-ensembling}

These are combined using exponential weighting:

$$a_t = \frac{\sum_{i=0}^{k-1} w_i \cdot A_t[i]}{\sum_{i=0}^{k-1} w_i}, \quad w_i = \exp(-m \cdot i)$$ {#eq-temporal-ensembling}
...
**Benefits:**
- **Smoothness**: Averages out prediction noise without introducing bias
- **Consistency**: Reduces modeling errors from any single prediction
- **Performance gain**: 3-4% improvement over no ensembling$^{[1]}$

## Training ACT {#sec-training}
...
From the [official repository](https://github.com/tonyzhaozh/act):
...
- Use **L1 loss** (not L2) for precise positioning tasks — L1 provides better gradients for exact alignment
- Set **β=10** for KL weight (balances reconstruction vs. regularization)
- CVAE is **essential for human data** (drops from 35% to 2% success without it)$^{[1]}$

::: {.callout-important}
## Key Insight: Train Past Loss Plateau
...
### Ablation Study Highlights {#sec-ablations}
...
## ACT vs Diffusion Policy {#sec-comparison}

Both ACT and Diffusion Policy (which we'll cover in [Part 2](../2-diffusion-policy/)) use action chunking, but differ in the generative model:
...
## Using ACT with LeRobot {#sec-lerobot}
...
**Recent extensions**:
- **Bi-ACT**: Incorporates force/torque feedback for contact-rich tasks
- **NL-ACT**: Adds natural language conditioning for instruction following
- **One ACT Play**: Learns from single demonstrations
- **3D-ACT**: Uses point cloud inputs instead of RGB images

## What's Next? {#sec-next}

In [Part 2](../2-diffusion-policy/), we'll explore **Diffusion Policy** in depth — understanding how diffusion models are adapted for action generation, why they handle multimodality better than VAEs, and how they compare to ACT in practice.
...
The combination of action chunking ([@eq-chunking-objective]), CVAE for handling demonstration variability ([@eq-cvae-loss]), and temporal ensembling for smoothness ([@eq-temporal-ensembling]) creates a powerful framework for learning precise manipulation from limited data.
...
[1] [Zhao, T. Z., et al. "Learning Fine-Grained Bimanual Manipulation with Low-Cost Hardware."](https://arxiv.org/abs/2304.13705) *Robotics: Science and Systems (RSS)*, 2023.

[2] [Chi, C., et al. "Diffusion Policy: Visuomotor Policy Learning via Action Diffusion."](https://arxiv.org/abs/2303.04137) *Robotics: Science and Systems (RSS)*, 2023.

[3] [Miller, G. A. "The Magical Number Seven, Plus or Minus Two: Some Limits on Our Capacity for Processing Information."](https://psychclassics.yorku.ca/Miller/) *Psychological Review* 63(2), 1956.

[4] [Camacho, E. F., & Alba, C. B. "Model Predictive Control."](https://link.springer.com/book/10.1007/978-0-85729-398-5) *Springer*, 2013.

[5] [Carion, N., et al. "End-to-End Object Detection with Transformers (DETR)."](https://arxiv.org/abs/2005.12872) *ECCV*, 2020.

[6] [Kingma, D. P., & Welling, M. "Auto-Encoding Variational Bayes."](https://arxiv.org/abs/1312.6114) *ICLR*, 2014.
...
- [The importance of action chunking in imitation learning](https://www.haonanyu.blog/post/action_chunking/) — Technical deep dive by Haonan Yu
- [Dissecting ACT](https://blog.phospho.ai/dissecting-action-chunking-with-transformers-act-precision-imitation-learning-for-robotic-manipulation/) — Detailed technical breakdown

...
- Simple to implement (no new architectures needed)
- Proven to work at scale (billions of parameters)

**Disadvantages**:
- Limited precision: ±0.5cm, ±3° quantization errors
- Poor for high-frequency dexterous control (autoregressive generation is slow)
...
**Advantages**:
- High-frequency control (50-100 Hz) for responsive manipulation
- No quantization errors — physically plausible smooth movements
- Naturally handles action chunking (recall [Part 1.5](../1.5-action-chunking/#sec-action-chunking))

**Disadvantages**:
- More complex training (requires learning continuous transformations)
- Slower inference (iterative generation through denoising steps)
- Can degrade language understanding if not carefully designed
...
::: {.callout-tip appearance="simple"}
## Intuition: Book Knowledge + Practice
...
1. **Semantic reasoning**: "Pick up the object that could serve as a hammer"
...
2. **Mathematical understanding**: "Move the banana to the sum of two plus one"
   - Computes: 2 + 1 = 3
   - Moves banana to location marked "3"

3. **Symbol recognition**: "Place the apple in the bowl marked with the same color"
   - Recognizes apple is red
   - Finds red-marked bowl
   - Executes color matching

4. **Comparative reasoning**: "Pick up the bag about to fall off the table"
...
::: {.callout-warning}
## Common Misconception: "VLAs Always Follow Language Instructions"
